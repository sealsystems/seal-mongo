...
const faultTolerantConnect = async function (connectionString, options) {
  ...
  const database = await retry(async () => {
    ...
    // Filter properties unknown by `mongodb` from options
    Object.keys(options).forEach((key) => {
      if (['connectionRetries', 'waitTimeBetweenRetries'].includes(key)) {
        return;
      }
    ...
    const client = await MongoClient.connect(connectionString, dbOptions);
